...
def evolutionary_algorithm(
    ...
    init_population = jax.random.uniform(init_key, (population_size,), minval = domain[0], maxval = domain[1])
    init_fitness = jax.vmap(objective)(population_size)
    ...
    def step(state):
        ...
        key, parent_key, pairing_key, crossover_key, mutation_key = jax.random.split(key, 6)
        ...
        alpha = jax.random.uniform(crossover_key, (population_size,))
    ...
    for generation in range(generations):
        ...
        print(f'NAJLEPSZY:{state["best"]:.2f};;NAJGORSZY:{state["worst"]:.2f};;ŚREDNIA:{state["mean"]:.2f}')
